...
export default function EditSiswaModal({ isOpen, onClose, onSiswaUpdated, siswaData }: EditSiswaModalProps) {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      await api.patch(`/siswa/${siswaData.id}`, {
        ...formData,
      ...
      if (axios.isAxiosError(error)) {
        setError(error.response?.data?.message || "Gagal mengupdate data siswa.");
      } else {
        setError("Gagal mengupdate data siswa. Terjadi kesalahan tidak terduga.");
      }
    } finally {
      setLoading(false);
    }
