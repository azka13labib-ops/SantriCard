...
export default function AddSiswaModal({ isOpen, onClose, onSiswaAdded }: { isOpen: boolean, onClose: () => void, onSiswaAdded: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        setError(error.response?.data?.message || "Gagal menambah siswa. Cek apakah NIS sudah dipakai.");
      } else {
        setError("Gagal menambah siswa. Terjadi kesalahan tidak terduga.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="pt-4 flex gap-3 justify-end">
            ...
            >
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Simpan & Buat Kartu"}
            </button>
          </div>
        </form>
      </div>
