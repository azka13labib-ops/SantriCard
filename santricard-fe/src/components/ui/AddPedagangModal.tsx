...
export default function AddPedagangModal({ isOpen, onClose, onPedagangAdded }: { isOpen: boolean, onClose: () => void, onPedagangAdded: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/pedagang", formData);
      onPedagangAdded();
      onClose();
    } catch (error: unknown) {
      console.error(error);
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="pt-2 border-t">
            <p className="text-xs text-gray-500 mb-3">Kredensial Login Pedagang:</p>
            <div>
