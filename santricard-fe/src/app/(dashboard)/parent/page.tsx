...
export default function DashboardOrtu() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (user.students && user.students.length > 0) {
        const currentSiswa = user.students[0];
        setStudent(currentSiswa);
        
        const histRes = await api.get(`/student/${currentSiswa.id}/histori`);
  ...
  const limitPercentage = Math.min(100, Math.max(0, (student.sisa_limit_hari_ini / student.limit_harian) * 100));
      ...
      <div className="md:col-span-5 lg:col-span-4 space-y-6">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h2 className="text-sm font-semibold">{student.nama}</h2>
              </div>
            ...
            <h2 className="text-4xl font-bold mt-4 mb-1">
              {formatRupiah(student.saldo_virtual)}
            </h2>
          </div>
        </div>
        ...
        <div className="bg-white border border-emerald-100 rounded-2xl p-5 shadow-sm relative overflow-hidden">
          ...
          <div className="flex justify-between items-end mb-2">
            <span className="text-2xl font-bold text-gray-900">{formatRupiah(student.sisa_limit_hari_ini)}</span>
          </div>
          ...
          <p className="text-xs text-gray-500">
            Dari total jatah {formatRupiah(student.limit_harian)} / hari
          </p>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm divide-y divide-gray-50">
          {histori.length > 0 ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500 text-sm">Belum ada riwayat transaction.</p>
            </div>
          )}
        </div>
      </div>
